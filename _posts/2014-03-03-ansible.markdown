...
Initially, I thought this task would be simple. But one should always know when you are dealing with provisioning tools and setup, that there will be continual problems to solve. I liken waiting for a provisioning run to complete worse than watching drying paint because at least with paintin, you paint and it dries. With provisioning, you have to wait for it to fail. It would be like watching paint dry and having to paint it all over again because it was the wrong color. 
...
The core functionality for this [playbook][ansible-playbook] is how it determines if a node is the first node and if it should be set to bootstrap (`wsrep_cluster_address=gcomm://`). Much of the functionality, I simply modified the work I had done in a Salt state for [Galera][Galera]. 
...
Check out the docker-galera and cluster-install repositories (the first two listed above). I usually pick a place such as ~/code, but wherever you like to do your work. 
...
Enter the directory containing the cluster-install repository and run the following:

    ~/code/cluster-install$ ../docker-galera/docker-launch-nodes.sh

At this point, you have 4 running containers -- 3 for [PXC][xtradb-cluster] (your [Galera][Galera] cluster) and 1 for [haproxy][haproxy] (this would be essentially any application that needs to connect to the cluster) and for this exercise, a hosts file that ansible will use.
...
At this point, you will have a proxy node that can connect to the cluster!


### Use the cluster

You will need to find out the IP address of your [HAProxy][haproxy] container:
...
This cluster setup is an example of the great things you can do with [Ansible][Ansible] and [Docker][Docker] and I intend on improving the playbook and setup. Some of which include:
...
The one thing I took away from building this was how much faster it is to deploy containers than virtual machine instances. This has profound implication for software packaging and testing.
